 ...
 */
@RestController
@RequestMapping("/api/sys/web-emr-supervise-info")
@Api(tags = "WebEmrSuperviseInfoController", description = "电子病历信息")
public class WebEmrSuperviseInfoController {

	@PostMapping(value = "/addWebEmrSuperviseInfo")
	@ApiResponses(value = {
	...
	public Map addWebEmrSuperviseInfo(String params) {
		WebEmrSuperviseInfo webEmrSuperviseInfo = null;
		try {
			webEmrSuperviseInfo = (WebEmrSuperviseInfo) JsonUtil.jsonToJava(params, WebEmrSuperviseInfo.class.getName());
		} catch (Exception e) {
			throw new HospitalException(ResultEnum.CAST_IS_VALID);
		}

		try {
			if (webEmrSuperviseInfo != null) {
				webEmrSuperviseInfo.insert();
			}
		} catch (Exception e) {
