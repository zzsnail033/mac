 ...
 */
@RestController
@RequestMapping("/api/sys/web-hospital-eqp-info")
@Api(tags = "WebHospitalEqpInfoController", description = "医院设备信息")
public class WebHospitalEqpInfoController {

	@PostMapping(value = "/addWebHospitalEqpInfo")
	@ApiResponses(value = {
	...
	public Map addWebHospitalEqpInfo(String params) {
		WebHospitalEqpInfo webHospitalEqpInfo = null;
		try {
			webHospitalEqpInfo = (WebHospitalEqpInfo) JsonUtil.jsonToJava(params, WebHospitalEqpInfo.class.getName());
		} catch (Exception e) {
			throw new HospitalException(ResultEnum.CAST_IS_VALID);
		}

		try {
			if (webHospitalEqpInfo != null) {
				webHospitalEqpInfo.insert();
			}
		} catch (Exception e) {
