 ...
 */
@RestController
@RequestMapping("/api/sys/sign-hospital-basic-info")
@Api(tags = "SignHospitalBasicInfoController", description = "合作医疗机构信息")
public class SignHospitalBasicInfoController {

	@PostMapping(value = "/addSignHospitalBasicInfo")
	@ApiResponses(value = {
	...
	public Map addSignHospitalBasicInfo(String params) {
		SignHospitalBasicInfo signHospitalBasicInfo = null;
		try {
			signHospitalBasicInfo = (SignHospitalBasicInfo) JsonUtil.jsonToJava(params, SignHospitalBasicInfo.class.getName());
		} catch (Exception e) {
			throw new HospitalException(ResultEnum.CAST_IS_VALID);
		}

		try {
			if (signHospitalBasicInfo != null) {
				signHospitalBasicInfo.insert();
			}
		} catch (Exception e) {
