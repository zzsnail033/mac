 ...
 */
@RestController
@RequestMapping("/api/sys/drug-delivery-supersive")
@Api(tags = "DrugDeliverySupersiveController", description = "处方流转")
public class DrugDeliverySupersiveController {

	@PostMapping(value = "/addDrugDeliverySupersive")
	@ApiResponses(value = {
	...
	public Map addDrugDeliverySupersive(String params) {
		DrugDeliverySupersive drugDeliverySupersive = null;
		try {
			drugDeliverySupersive = (DrugDeliverySupersive) JsonUtil.jsonToJava(params, DrugDeliverySupersive.class.getName());
		} catch (Exception e) {
			throw new HospitalException(ResultEnum.CAST_IS_VALID);
		}

		try {
			if (drugDeliverySupersive != null) {
				drugDeliverySupersive.insert();
			}
		} catch (Exception e) {
