...
import com.nykj.dataconversion.sys.entity.WebAppointmentInfo;
 ...
 */
@RestController
@RequestMapping("/api/sys/web-appointment-info")
public class WebAppointmentInfoController {

	@PostMapping(value = "/addWebAppointmentInfo")
	@ApiResponses(value = {
	...
	public Map addWebAppointmentInfo(String params) {
		WebAppointmentInfo webAppointmentInfo = null;
		try {
			webAppointmentInfo = (WebAppointmentInfo) JsonUtil.jsonToJava(params, WebAppointmentInfo.class.getName());
		} catch (Exception e) {
			throw new HospitalException(ResultEnum.CAST_IS_VALID);
		}

		try {
			if (webAppointmentInfo != null) {
				webAppointmentInfo.insert();
			}
		} catch (Exception e) {
